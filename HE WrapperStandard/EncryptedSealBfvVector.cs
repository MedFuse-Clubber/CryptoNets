...
namespace HE_WrapperStandard
{
    public class EncryptedSealBfvEnvironment : IComputationEnvironment
    {
        ...
        BigInteger Reminder(BigInteger n1, BigInteger n2, BigInteger res)
        {
            BigInteger q = n1 / n2;
            res = n1 - q * n2;
            return res;
        ...
        {
            var factors = Environments.Select(e => new BigInteger(e.parameters.PlainModulus.Value));
            var uIntBigFactor = factors.Aggregate((n1, n2) => n1 * n2);
            bigFactor = uIntBigFactor;
            var minors = factors.Select(p => uIntBigFactor / p).ToList();
            var t = new BigInteger();
            BigInteger[] ys = minors.Zip(factors, (m, p) => Reminder(m, p, t).ModuloInvert(p)).ToArray();
;
            preComputedCoefficients = ys.Zip(minors, (y, m) => y * m).Select(v => v).ToArray();
        }

        public void GenerateEncryptionKeys(ulong[] primes, ulong n, int DecompositionBitCount, int GaloisDecompositionBitCount, int SmallModulusCount = -1, IEnumerable<uint> DefaultGaloisElements = null)
        {
