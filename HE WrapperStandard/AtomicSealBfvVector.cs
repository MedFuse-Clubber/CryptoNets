...
namespace HE_WrapperStandard
{
    public class AtomicSealBfvEncryptedEnvironment : IComputationEnvironment
    {
        ...
        public void SetKeys(KeyGenerator keys, IEnumerable<uint> GaloisElements, int DecompositionBitCount = 0, int GaloisDecompositionBitCount = 0)
        {
            evaluator = new Evaluator(context);
            // Create and assign public key
            keys.CreatePublicKey(out publicKey);
        ...
        public static EncryptionParameters Parms(EncryptionParameters parameters, ulong t, ulong n, int SmallModulusCount)
        {
        ...
        public static EncryptionParameters Parms(ulong t, ulong n, List<Modulus> coefModulus = null)
        {
            var parms = new EncryptionParameters(SchemeType.BFV)
            {
                PlainModulus = new Modulus(t),
        ...
        public void GenerateEncryptionKeys(EncryptionParameters parms, int DecompositionBitCount, int GaloisDecompositionBitCount , IEnumerable<uint> GaloisElement)
        {
            this.parameters = parms;
            context = new SEALContext(parameters);
            var keyGenerator = new KeyGenerator(context);
            SetKeys(keyGenerator, GaloisElement);
        }
        ...
        {
            GenerateEncryptionKeys(Convert.ToUInt64(hexPrime, 16), n, DecompositionBitCount, GaloisDecompositionBitCount, SmallModulusCount, EncryptedSealBfvFactory.DefaultGaloisElements);
        }
...
//            if (vecs[0].Dim > blockSize / 2)  throw new Exception("no support for inteleave of data that is greater than half block");
        ...
        public Vector<double> Decrypt(IComputationEnvironment env)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                {
                    // sparse format
                    var w = new BigInteger(Convert.ToByte(plain.ToString()));
                    var bi = w;
                    var v = (double)bi;
